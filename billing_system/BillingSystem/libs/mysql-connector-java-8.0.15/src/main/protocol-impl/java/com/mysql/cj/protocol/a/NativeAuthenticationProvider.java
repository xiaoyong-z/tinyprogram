...
public class NativeAuthenticationProvider implements AuthenticationProvider<NativePacketPayload> {
     ...
     * given database as a given MySQL userDao. This can happen upon initial
     ...
     * @param user
     *            the MySQL userDao account to log into
     * @param password
     *            authentication data for the userDao account (depends
        ...
        while (0 < counter--) {
            ...
            if (!done) {
                ...
                if (challenge != null) {
                    ...
                    // We allow the userDao to configure whether or not they want to support multiple queries (by default, this is disabled).
                    ...
                    if (plugin == null) {
                         ...
                         * successfully. If default is 'sha256_password' then the authentication will fail as expected. In both cases userDao's password won't be
    ...
    /**
     * Re-authenticates as the given userDao and password
     * 
     * @param serverSession
