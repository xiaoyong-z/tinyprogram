...
public class NativeSession extends CoreSession implements Serializable {
    ...
    /**
     * Actual collation index to mysql charset name map of userDao defined charsets for given server URLs.
     */
    private static final Map<String, Map<Integer, String>> customIndexToCharsetMapByUrl = new HashMap<>();

    /**
     * Actual mysql charset name to mblen map of userDao defined charsets for given server URLs.
     */
    ...
    // TODO: this method should not be used in userDao-level APIs
    ...
    public boolean configureClientCharacterSet(boolean dontCheckServerMatch) {
        ...
        try {
            ...
            //
            // We know how to deal with any charset coming back from the database, so tell the server not to do conversion if the userDao hasn't 'forced' a
            ...
            // Failsafe, make sure that the driver's notion of character encoding matches what the userDao has specified.
            //
            this.characterEncoding.setValue(realJavaEncoding);
        }
