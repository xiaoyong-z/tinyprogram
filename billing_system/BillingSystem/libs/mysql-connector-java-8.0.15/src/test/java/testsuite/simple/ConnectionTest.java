...
public class ConnectionTest extends BaseTestCase {
    ...
    public void testDeadlockDetection() throws Exception {
        try {
            ...
            if (sqlEx.getMessage().indexOf("PROCESS privilege") != -1) {
                fail("This test requires userDao with process privilege");
            }
    ...
    public void testNewHostParsing() throws Exception {
        ...
        String newUrl = String.format("jdbc:mysql://address=(protocol=tcp)(host=%s)(port=%s)(userDao=%s)(password=%s)/%s", TestUtils.encodePercent(host), port,
    ...
    public void testDriverConnectPropertiesPrecedence() throws Exception {
        assertThrows(SQLException.class, "Access denied for userDao 'dummy'@'[^']+' \\(using password: YES\\)", new Callable<Void>() {
            public Void call() throws Exception {
    ...
    /**
     * Test authentication with a userDao that requires an SSL connection.
     * 
    ...
    /**
     * Test authentication with a userDao that requires an SSL connection and an authorized client certificate.
     * 
     * This test requires the CA truststore and the client keystore available in src/test/config/ssl-test-certs.
     * The server needs to be configured with the CA and server certificates from src/test/config/ssl-test-certs.
     */
